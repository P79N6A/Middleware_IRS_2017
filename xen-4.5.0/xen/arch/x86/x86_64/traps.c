...
void toggle_guest_mode(struct vcpu *v)
{
    ...
    if ( v->arch.pv_vcpu.need_update_runstate_area &&
        ...
        TRACE_4D(TRC_SCHED_TOGGLE, v->domain->domain_id, v->vcpu_id,
                TF_kernel_mode, NOW());
    ...
    /* Mod by YZhao (arch/x86/x86_64/traps.c)
     * 1st: 9/24/2015
     *
     *

    if (v->arch.pv_vcpu.need_update_ctxt_area &&
            update_ctxt_area(v)) {
        v->arch.pv_vcpu.need_update_ctxt_area = 0;
    }
