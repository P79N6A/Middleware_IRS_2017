...
/* Mod by YZhao (arch/x86/domain.c)
 * 1st: 10/7/2015
 */
...
bool_t update_runstate_area(struct vcpu *v)
{
    ...
    /* Mod by YZhao (arch/x86/domain.c)
     * 1st: 10/7/2015
     * check the value of rc so that we can know
     * whether execution of the __copy_to_guest(...) succeeded.
     *

    TRACE_5D(TRC_SCHED_UPDATE_RUNSTATE, v->domain->domain_id, v->vcpu_id, 
                rc, sizeof(v->runstate), NOW());
...
/* Mod by YZhao (arch/x86/domain.c)
 * 1st: 9/24/2015
 * Update per-VPCU guest context info shared memory area.
 *

bool_t update_ctxt_area(struct vcpu* v) {
    bool_t rc;
    smap_check_policy_t smap_policy;

    if (guest_handle_is_null(ctxt_guest(v))) {
        return 1;
    }
    smap_policy = smap_policy_change(v, SMAP_CHECK_ENABLED);
    ...
    if (has_32bit_shinfo(v->domain)) {
        ...
        //struct compat_vcpu_ctxt_info info;
        //XLAT_vcpu_ctxt_info(&info, &v->ctxt);
        //__copy_to_guest(v->ctxt_guest.compat, &info, 1);
        
        rc = 1;
    } else {
        rc = __copy_to_guest(ctxt_guest(v), &v->ctxt, 1) != 
            sizeof(v->ctxt);
    }
...
static void _update_ctxt_area(struct vcpu* v) {
    if (!update_ctxt_area(v) && is_pv_vcpu(v) 
            && !(v->arch.flags & TF_kernel_mode)) {
        v->arch.pv_vcpu.need_update_ctxt_area = 1;
    }
}
...
static void _update_runstate_area(struct vcpu *v)
{
    if ( !update_runstate_area(v) && is_pv_vcpu(v) &&
        ...
        /* Mod by YZhao (arch/x86/domain.c)
         * 1st: 10/8/2015
         * checking whether the value of need_update_runstate_area 
         * is set at here.
         *

        TRACE_3D(TRC_SCHED_UPDATE_AREA, v->domain->domain_id, v->vcpu_id, 
                    NOW());
...
static void __context_switch(void)
{
    ...
    if (((p->ctxt_counter % 10000 == 0) && (p->domain->domain_id == 1 || p->domain->domain_id == 2))) {
        printk("Dom: %d, vcpu: %d RIP: %04x:[%016lx]\n", 
                p->domain->domain_id, p->vcpu_id, stack_regs->cs, stack_regs->rip);
    }
    if (((n->ctxt_counter % 10000 == 0) && (n->domain->domain_id == 1 || n->domain->domain_id == 2))) {
        printk("Dom: %d, vcpu: %d RIP: %04x:[%016lx]\n", 
                n->domain->domain_id, n->vcpu_id, stack_regs->cs, stack_regs->rip);
    }

    if (p != n) {
        if ((p->domain->domain_id != 0)
                && (p->domain->domain_id != 32767)) {
            memcpy(&p->ctxt, stack_regs, CTXT_SWITCH_STACK_BYTES);
            _update_ctxt_area(p);
        }
    }
