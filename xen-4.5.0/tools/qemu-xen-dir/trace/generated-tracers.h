...
{
    bool _state = trace_event_get_state(TRACE_BDRV_AIO_WRITE_ZEROES);
    if (_state) {
        fprintf(stderr, "bdrv_aio_write_zeroes " "bs %p sector_num %"PRId64" nb_sectors %d flags %#x opaque %p" "\n" , bs, sector_num, nb_sectors, flags, opaque);
    }
}
...
static inline void trace_bdrv_co_write_zeroes(void * bs, int64_t sector_num, int nb_sector, int flags)
{
    ...
    if (_state) {
        fprintf(stderr, "bdrv_co_write_zeroes " "bs %p sector_num %"PRId64" nb_sectors %d flags %#x" "\n" , bs, sector_num, nb_sector, flags);
    }
}
...
static inline void trace_mirror_before_sleep(void * s, int64_t cnt, int synced, uint64_t delay_ns)
{
    ...
    if (_state) {
        fprintf(stderr, "mirror_before_sleep " "s %p dirty count %"PRId64" synced %d delay %"PRIu64"ns" "\n" , s, cnt, synced, delay_ns);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_PAIO_SUBMIT_CO);
    if (_state) {
        fprintf(stderr, "paio_submit_co " "sector_num %"PRId64" nb_sectors %d type %d" "\n" , sector_num, nb_sectors, type);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_USB_EHCI_PORT_SUSPEND);
    if (_state) {
        fprintf(stderr, "usb_ehci_port_suspend " "port #%d" "\n" , port);
    }
}
static inline void trace_usb_ehci_port_wakeup(uint32_t port)
{
    bool _state = trace_event_get_state(TRACE_USB_EHCI_PORT_WAKEUP);
    if (_state) {
        fprintf(stderr, "usb_ehci_port_wakeup " "port #%d" "\n" , port);
    }
}
static inline void trace_usb_ehci_port_resume(uint32_t port)
{
    bool _state = trace_event_get_state(TRACE_USB_EHCI_PORT_RESUME);
    if (_state) {
        fprintf(stderr, "usb_ehci_port_resume " "port #%d" "\n" , port);
    }
}
...
static inline void trace_usb_xhci_slot_address(uint32_t slotid, const char * port)
{
    ...
    if (_state) {
        fprintf(stderr, "usb_xhci_slot_address " "slotid %d, port %s" "\n" , slotid, port);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_USB_XHCI_EP_STATE);
    if (_state) {
        fprintf(stderr, "usb_xhci_ep_state " "slotid %d, epid %d, %s -> %s" "\n" , slotid, epid, os, ns);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_USB_DESC_MSOS);
    if (_state) {
        fprintf(stderr, "usb_desc_msos " "dev %d msos, index 0x%x, len %d, ret %d" "\n" , addr, index, len, ret);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_USB_HUB_STATUS_REPORT);
    if (_state) {
        fprintf(stderr, "usb_hub_status_report " "dev %d, status 0x%x" "\n" , addr, status);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_SYSTEM_WAKEUP_REQUEST);
    if (_state) {
        fprintf(stderr, "system_wakeup_request " "reason=%d" "\n" , reason);
    }
}
...
static inline void trace_qcow2_alloc_clusters_offset(void * co, uint64_t offset, int num)
{
    ...
    if (_state) {
        fprintf(stderr, "qcow2_alloc_clusters_offset " "co %p offset %" PRIx64 " num %d" "\n" , co, offset, num);
    }
}
static inline void trace_qcow2_handle_copied(void * co, uint64_t guest_offset, uint64_t host_offset, uint64_t bytes)
{
    ...
    if (_state) {
        fprintf(stderr, "qcow2_handle_copied " "co %p guest_offset %" PRIx64 " host_offset %" PRIx64 " bytes %" PRIx64 "\n" , co, guest_offset, host_offset, bytes);
    }
}
static inline void trace_qcow2_handle_alloc(void * co, uint64_t guest_offset, uint64_t host_offset, uint64_t bytes)
{
    ...
    if (_state) {
        fprintf(stderr, "qcow2_handle_alloc " "co %p guest_offset %" PRIx64 " host_offset %" PRIx64 " bytes %" PRIx64 "\n" , co, guest_offset, host_offset, bytes);
    }
}
static inline void trace_qcow2_do_alloc_clusters_offset(void * co, uint64_t guest_offset, uint64_t host_offset, int nb_clusters)
{
    ...
    if (_state) {
        fprintf(stderr, "qcow2_do_alloc_clusters_offset " "co %p guest_offset %" PRIx64 " host_offset %" PRIx64 " nb_clusters %d" "\n" , co, guest_offset, host_offset, nb_clusters);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_MEGASAS_DCMD_LD_LIST_QUERY);
    if (_state) {
        fprintf(stderr, "megasas_dcmd_ld_list_query " "scmd %d: DCMD LD list query flags %x" "\n" , cmd, flags);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_CONSOLE_PUTCHAR_CSI);
    if (_state) {
        fprintf(stderr, "console_putchar_csi " "escape sequence CSI%d;%d%c, %d parameters" "\n" , esc_param0, esc_param1, ch, nb_esc_params);
    }
}
static inline void trace_console_putchar_unhandled(int ch)
{
    bool _state = trace_event_get_state(TRACE_CONSOLE_PUTCHAR_UNHANDLED);
    if (_state) {
        fprintf(stderr, "console_putchar_unhandled " "unhandled escape character '%c'" "\n" , ch);
    }
}
...
static inline void trace_gd_switch(int width, int height)
{
    bool _state = trace_event_get_state(TRACE_GD_SWITCH);
    if (_state) {
        fprintf(stderr, "gd_switch " "width=%d, height=%d" "\n" , width, height);
    }
}
static inline void trace_gd_update(int x, int y, int w, int h)
{
    bool _state = trace_event_get_state(TRACE_GD_UPDATE);
    if (_state) {
        fprintf(stderr, "gd_update " "x=%d, y=%d, w=%d, h=%d" "\n" , x, y, w, h);
    }
}
static inline void trace_gd_key_event(int gdk_keycode, int qemu_keycode, const char * action)
{
    bool _state = trace_event_get_state(TRACE_GD_KEY_EVENT);
    if (_state) {
        fprintf(stderr, "gd_key_event " "translated GDK keycode %d to QEMU keycode %d (%s)" "\n" , gdk_keycode, qemu_keycode, action);
    }
}
static inline void trace_input_event_key_number(int conidx, int number, bool down)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_KEY_NUMBER);
    if (_state) {
        fprintf(stderr, "input_event_key_number " "con %d, key number 0x%x, down %d" "\n" , conidx, number, down);
    }
}
static inline void trace_input_event_key_qcode(int conidx, const char * qcode, bool down)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_KEY_QCODE);
    if (_state) {
        fprintf(stderr, "input_event_key_qcode " "con %d, key qcode %s, down %d" "\n" , conidx, qcode, down);
    }
}
static inline void trace_input_event_btn(int conidx, const char * btn, bool down)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_BTN);
    if (_state) {
        fprintf(stderr, "input_event_btn " "con %d, button %s, down %d" "\n" , conidx, btn, down);
    }
}
static inline void trace_input_event_rel(int conidx, const char * axis, int value)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_REL);
    if (_state) {
        fprintf(stderr, "input_event_rel " "con %d, axis %s, value %d" "\n" , conidx, axis, value);
    }
}
static inline void trace_input_event_abs(int conidx, const char * axis, int value)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_ABS);
    if (_state) {
        fprintf(stderr, "input_event_abs " "con %d, axis %s, value 0x%x" "\n" , conidx, axis, value);
    }
}
static inline void trace_input_event_sync(void)
{
    bool _state = trace_event_get_state(TRACE_INPUT_EVENT_SYNC);
    if (_state) {
        fprintf(stderr, "input_event_sync " "" "\n" );
    }
}
static inline void trace_input_mouse_mode(int absolute)
{
    bool _state = trace_event_get_state(TRACE_INPUT_MOUSE_MODE);
    if (_state) {
        fprintf(stderr, "input_mouse_mode " "absolute %d" "\n" , absolute);
    }
}
...
static inline void trace_savevm_section_start(const char * id, unsigned int section_id)
{
    ...
    if (_state) {
        fprintf(stderr, "savevm_section_start " "%s, section_id %u" "\n" , id, section_id);
    }
}
static inline void trace_savevm_section_end(const char * id, unsigned int section_id)
{
    ...
    if (_state) {
        fprintf(stderr, "savevm_section_end " "%s, section_id %u" "\n" , id, section_id);
    }
}
static inline void trace_savevm_state_begin(void)
{
    bool _state = trace_event_get_state(TRACE_SAVEVM_STATE_BEGIN);
    if (_state) {
        fprintf(stderr, "savevm_state_begin " "" "\n" );
    }
}
static inline void trace_savevm_state_iterate(void)
{
    bool _state = trace_event_get_state(TRACE_SAVEVM_STATE_ITERATE);
    if (_state) {
        fprintf(stderr, "savevm_state_iterate " "" "\n" );
    }
}
static inline void trace_savevm_state_complete(void)
{
    bool _state = trace_event_get_state(TRACE_SAVEVM_STATE_COMPLETE);
    if (_state) {
        fprintf(stderr, "savevm_state_complete " "" "\n" );
    }
}
static inline void trace_savevm_state_cancel(void)
{
    bool _state = trace_event_get_state(TRACE_SAVEVM_STATE_CANCEL);
    if (_state) {
        fprintf(stderr, "savevm_state_cancel " "" "\n" );
    }
}
static inline void trace_vmstate_save(const char * idstr, const char * vmsd_name)
{
    bool _state = trace_event_get_state(TRACE_VMSTATE_SAVE);
    if (_state) {
        fprintf(stderr, "vmstate_save " "%s, %s" "\n" , idstr, vmsd_name);
    }
}
static inline void trace_vmstate_load(const char * idstr, const char * vmsd_name)
{
    bool _state = trace_event_get_state(TRACE_VMSTATE_LOAD);
    if (_state) {
        fprintf(stderr, "vmstate_load " "%s, %s" "\n" , idstr, vmsd_name);
    }
}
static inline void trace_vmstate_load_field_error(const char * field, int ret)
{
    bool _state = trace_event_get_state(TRACE_VMSTATE_LOAD_FIELD_ERROR);
    if (_state) {
        fprintf(stderr, "vmstate_load_field_error " "field \"%s\" load failed, ret = %d" "\n" , field, ret);
    }
}
static inline void trace_qemu_announce_self_iter(const char * mac)
{
    bool _state = trace_event_get_state(TRACE_QEMU_ANNOUNCE_SELF_ITER);
    if (_state) {
        fprintf(stderr, "qemu_announce_self_iter " "%s" "\n" , mac);
    }
}
static inline void trace_qemu_file_fclose(void)
{
    bool _state = trace_event_get_state(TRACE_QEMU_FILE_FCLOSE);
    if (_state) {
        fprintf(stderr, "qemu_file_fclose " "" "\n" );
    }
}
...
static inline void trace_qxl_io_write(int qid, const char * mode, uint64_t addr, const char * aname, uint64_t val, unsigned size, int async)
{
    ...
    if (_state) {
        fprintf(stderr, "qxl_io_write " "%d %s addr=%"PRIu64 " (%s) val=%"PRIu64" size=%u async=%d" "\n" , qid, mode, addr, aname, val, size, async);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_SPAPR_IOMMU_PUT);
    if (_state) {
        fprintf(stderr, "spapr_iommu_put " "liobn=%"PRIx64" ioba=0x%"PRIx64" tce=0x%"PRIx64" ret=%"PRId64 "\n" , liobn, ioba, tce, ret);
    }
}
static inline void trace_spapr_iommu_get(uint64_t liobn, uint64_t ioba, uint64_t ret, uint64_t tce)
{
    bool _state = trace_event_get_state(TRACE_SPAPR_IOMMU_GET);
    if (_state) {
        fprintf(stderr, "spapr_iommu_get " "liobn=%"PRIx64" ioba=0x%"PRIx64" ret=%"PRId64" tce=0x%"PRIx64 "\n" , liobn, ioba, ret, tce);
    }
}
static inline void trace_spapr_iommu_xlate(uint64_t liobn, uint64_t ioba, uint64_t tce, unsigned perm, unsigned pgsize)
{
    bool _state = trace_event_get_state(TRACE_SPAPR_IOMMU_XLATE);
    if (_state) {
        fprintf(stderr, "spapr_iommu_xlate " "liobn=%"PRIx64" 0x%"PRIx64" -> 0x%"PRIx64" perm=%u mask=%x" "\n" , liobn, ioba, tce, perm, pgsize);
    }
}
static inline void trace_spapr_iommu_new_table(uint64_t liobn, void * tcet, void * table, int fd)
{
    bool _state = trace_event_get_state(TRACE_SPAPR_IOMMU_NEW_TABLE);
    if (_state) {
        fprintf(stderr, "spapr_iommu_new_table " "liobn=%"PRIx64" tcet=%p table=%p fd=%d" "\n" , liobn, tcet, table, fd);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_CSS_ADAPTER_INTERRUPT);
    if (_state) {
        fprintf(stderr, "css_adapter_interrupt " "CSS: adapter I/O interrupt (isc %x)" "\n" , isc);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_FLIC_CREATE_DEVICE);
    if (_state) {
        fprintf(stderr, "flic_create_device " "flic: create device failed %d" "\n" , err);
    }
}
static inline void trace_flic_no_device_api(int err)
{
    bool _state = trace_event_get_state(TRACE_FLIC_NO_DEVICE_API);
    if (_state) {
        fprintf(stderr, "flic_no_device_api " "flic: no Device Contral API support %d" "\n" , err);
    }
}
static inline void trace_flic_reset_failed(int err)
{
    bool _state = trace_event_get_state(TRACE_FLIC_RESET_FAILED);
    if (_state) {
        fprintf(stderr, "flic_reset_failed " "flic: reset failed %d" "\n" , err);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_MIGRATE_FD_CLEANUP);
    if (_state) {
        fprintf(stderr, "migrate_fd_cleanup " "" "\n" );
    }
}
static inline void trace_migrate_fd_error(void)
{
    bool _state = trace_event_get_state(TRACE_MIGRATE_FD_ERROR);
    if (_state) {
        fprintf(stderr, "migrate_fd_error " "" "\n" );
    }
}
static inline void trace_migrate_fd_cancel(void)
{
    bool _state = trace_event_get_state(TRACE_MIGRATE_FD_CANCEL);
    if (_state) {
        fprintf(stderr, "migrate_fd_cancel " "" "\n" );
    }
}
static inline void trace_migrate_pending(uint64_t size, uint64_t max)
{
    bool _state = trace_event_get_state(TRACE_MIGRATE_PENDING);
    if (_state) {
        fprintf(stderr, "migrate_pending " "pending size %" PRIu64 " max %" PRIu64 "\n" , size, max);
    }
}
static inline void trace_migrate_transferred(uint64_t tranferred, uint64_t time_spent, double bandwidth, uint64_t size)
{
    bool _state = trace_event_get_state(TRACE_MIGRATE_TRANSFERRED);
    if (_state) {
        fprintf(stderr, "migrate_transferred " "transferred %" PRIu64 " time_spent %" PRIu64 " bandwidth %g max_size %" PRId64 "\n" , tranferred, time_spent, bandwidth, size);
    }
}
static inline void trace_kvm_ioctl(int type, void * arg)
{
    ...
    if (_state) {
        fprintf(stderr, "kvm_ioctl " "type 0x%x, arg %p" "\n" , type, arg);
    }
}
static inline void trace_kvm_vm_ioctl(int type, void * arg)
{
    ...
    if (_state) {
        fprintf(stderr, "kvm_vm_ioctl " "type 0x%x, arg %p" "\n" , type, arg);
    }
}
static inline void trace_kvm_vcpu_ioctl(int cpu_index, int type, void * arg)
{
    ...
    if (_state) {
        fprintf(stderr, "kvm_vcpu_ioctl " "cpu_index %d, type 0x%x, arg %p" "\n" , cpu_index, type, arg);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_KVM_DEVICE_IOCTL);
    if (_state) {
        fprintf(stderr, "kvm_device_ioctl " "dev fd %d, type 0x%x, arg %p" "\n" , fd, type, arg);
    }
}
static inline void trace_kvm_failed_spr_set(int str, const char * msg)
{
    bool _state = trace_event_get_state(TRACE_KVM_FAILED_SPR_SET);
    if (_state) {
        fprintf(stderr, "kvm_failed_spr_set " "Warning: Unable to set SPR %d to KVM: %s" "\n" , str, msg);
    }
}
static inline void trace_kvm_failed_spr_get(int str, const char * msg)
{
    bool _state = trace_event_get_state(TRACE_KVM_FAILED_SPR_GET);
    if (_state) {
        fprintf(stderr, "kvm_failed_spr_get " "Warning: Unable to retrieve SPR %d from KVM: %s" "\n" , str, msg);
    }
}
...
{
    bool _state = trace_event_get_state(TRACE_PCI_CFG_READ);
    if (_state) {
        fprintf(stderr, "pci_cfg_read " "%s %02u:%u @0x%x -> 0x%x" "\n" , dev, devid, fnid, offs, val);
    }
}
static inline void trace_pci_cfg_write(const char * dev, unsigned devid, unsigned fnid, unsigned offs, unsigned val)
{
    bool _state = trace_event_get_state(TRACE_PCI_CFG_WRITE);
    if (_state) {
        fprintf(stderr, "pci_cfg_write " "%s %02u:%u @0x%x <- 0x%x" "\n" , dev, devid, fnid, offs, val);
    }
}
static inline void trace_kvm_failed_reg_get(uint64_t id, const char * msg)
{
    bool _state = trace_event_get_state(TRACE_KVM_FAILED_REG_GET);
    if (_state) {
        fprintf(stderr, "kvm_failed_reg_get " "Warning: Unable to retrieve ONEREG %" PRIu64 " from KVM: %s" "\n" , id, msg);
    }
}
static inline void trace_kvm_failed_reg_set(uint64_t id, const char * msg)
{
    bool _state = trace_event_get_state(TRACE_KVM_FAILED_REG_SET);
    if (_state) {
        fprintf(stderr, "kvm_failed_reg_set " "Warning: Unable to set ONEREG %" PRIu64 " to KVM: %s" "\n" , id, msg);
    }
}
